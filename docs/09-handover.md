...
## Addendum, 21 September 2026: prototype screens, bring-your-own, legal v1.2 and v1.3, button targets

- **App screens.** Every phone frame on the site now shows a screen of the PRM v6 prototype (`prototype-v6/linkist_prm_v6.html`, outside the repo) or one of the owner's public-profile samples. `pnpm capture:v6` opens each screen in headless Chromium in the dark theme and writes `public/screens/v6-*.png` (780 x 1688) plus a 4:3 crop under `public/screens/crops/`; the samples were resized by hand into `public/screens/profile-*.webp` and `public/assets/cards/sample-*.webp` (masters under `public/assets/masters/samples/`, git-ignored). The registry is `src/content/design.ts`; every frame keeps the Design preview badge until real captures replace them (C2).
- **Home page.** "How Linkist works" is three stage cards and "Use cases" five picture cards, both heading-only, after the owner's homepage prototype (D51). The hero shows a phone with a profile sample and its Signature card; the composite scene is gone.
- **New page.** `/bring-your-own` (footer, hero link, cards page). Its buttons open nfctools.linkist.ai and prm.linkist.ai/quick-profile.
- **Legal.** The Linkist Terms and Privacy v1.0 (22 September, D53) replaced the v1.2 terms and v1.3 privacy policy of the day before. Word files live in `../legal/` (outside the repo) and are converted by `pnpm legal:docx <file> <terms|privacy>`; `terms` is the whole document, `privacy` is Part 2 on its own. /ai, /security, the help centre and the about page were rewritten against it (T and P citations). Open items C25 to C27.
- **Buttons.** `FREE_PROFILE_URL` (Create Free Profile, Start free, Get the App, nav and footer) is `prm.linkist.ai/quick-profile`; `GET_CARD_URL` and `STORE_URL` (Get Linkist NFC, Get NFC Card, every tier and bundle button) are `prm.linkist.ai/store/start`. Sign in and the `/app` and `/sign-in` redirects still use `/UnifiedAuth`. Override with `NEXT_PUBLIC_FREE_PROFILE_URL` and `NEXT_PUBLIC_GET_CARD_URL`.
- **Lighthouse** has not been re-run since 16 September; the owner asked for it to run only on request.
