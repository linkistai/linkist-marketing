...
- **Vercel**: project `linkist-marketing` in team `bettroi-website` (project id `prj_BxNTNh2RfTIw2vAkXv9hX2GHHX7E`), created on 11 September 2026 (D33). Root directory `.`, framework Next.js, install and build commands left at their defaults (`pnpm install`, `next build`), Node 22.x from the `engines` field in `package.json`. The project has its environment variables (below) and no deployment yet.
- **First production deploy**: one command from this folder, about 3 minutes. The build machine's assistant session was not permitted to publish, so the owner runs it:
...
- **Another Vercel project?** On 10 September the owner showed a Vercel build log for this repository failing with `Invalid URL, input: ''` (fixed the same day, `coerceUrl` in `src/lib/site.ts`). That project was not in team `bettroi-website`. If it exists elsewhere, either delete it or point it at the same repository with the same variables, so there is one production site.
...
4. Redeploy (`vercel deploy --prod --yes --scope bettroi-website`, or a push once C23 is done).
