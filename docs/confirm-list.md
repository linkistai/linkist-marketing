...
| C8 | AI: which model or provider, what data is sent, what is stored, how a person switches it off, and what an AI credit buys (the billing hub sells top-ups). | The `/ai` page, the privacy pages and the help centre's AI and data category. | Not built until answered. |
| C9 | Security: hosting provider and region, sub-processors by name, access controls, export and deletion in the app, any attestation. The privacy policy names Stripe and categories only. | The `/security` page and the legal sub-processor list. | Not built until answered; the site makes no compliance claim meanwhile (D10). |
