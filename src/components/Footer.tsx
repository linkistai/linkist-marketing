...
import { COMPANY, EXTERNAL_PRIVACY_URL, EXTERNAL_TERMS_URL, START_URL, SUPPORT_EMAIL } from '@/lib/site';
...
const COLUMNS: { title: string; links: { href: string; label: string; external?: boolean }[] }[] = [
  ...
  },
  {
    title: 'Company',
    links: [
      { href: START_URL, label: 'Sign in', external: true },
...
/**
 * Footer: four columns, the community sign-up, the motion switch (brief 6), cookie choices and the
 * company line. The newsletter keeps its own row below the tablet breakpoint (Grownz audit finding).
 */
    ...
    <footer className="footer-clearance border-t border-line" style={{ background: 'var(--color-bg)' }}>
      <div className="container grid gap-12 py-16 lg:grid-cols-[1.4fr_repeat(4,1fr)]">
        <div className="flex flex-col gap-4">
          ...
          <a href={START_URL} className="btn btn--primary btn--sm self-start">
            Start free
          </a>
      ...
      <div className="container flex flex-col gap-3 border-t border-line py-6 text-xs text-muted md:flex-row md:items-center md:justify-between">
        <p>
          © {new Date().getFullYear()} {COMPANY}. All rights reserved. Linkist is a web app at prm.linkist.ai.
        </p>
