...
export function GET() {
  const lines = [
    ...
    '',
    '## Facts',
    `- Plans (USD, AED in brackets): ${PLANS.map((p) => `${p.name} ${p.monthly === 0 ? 'free' : `$${p.monthly} (AED ${p.aed.monthly}) a month${p.perUser ? ' per user' : ''}${p.team ? `, $${p.team.usd.yearly} (AED ${p.team.aed.yearly.toLocaleString('en-AE')}) a year for ${p.minUsers} users` : p.yearly ? `, $${p.yearly} (AED ${p.aed.yearly}) a year` : ''}${p.minUsers ? `, minimum ${p.minUsers} users` : ''}`}`).join('; ')}. Enterprise is interest only.`,
